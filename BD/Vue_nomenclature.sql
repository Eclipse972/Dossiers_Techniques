...
-- Généré le : Ven 17 Janvier 2020 à 21:01
...
CREATE ALGORITHM=UNDEFINED DEFINER=`dossiers.techniques`@`172.20.%` SQL SECURITY DEFINER VIEW dossiers_techniques.Vue_nomenclature AS select dossiers_techniques.Supports.ID AS support_ID,concat(_utf8'<td>',cast(dossiers_techniques.Pieces.repere as char(2) charset utf8),_utf8'</td>') AS repere,concat(_latin1'<td><a href="Supports/',dossiers_techniques.Supports.dossier,_latin1'/fichiers/',dossiers_techniques.Pieces.fichier,convert(if((dossiers_techniques.Pieces.assemblage = 0),_utf8'.EPRT',_utf8'.EASM') using latin1),_latin1'"><img src="Supports/',dossiers_techniques.Supports.dossier,_latin1'/images/',dossiers_techniques.Pieces.fichier,_latin1'.png" alt="',dossiers_techniques.Pieces.nom,_latin1'"></a>',_latin1'</td>') AS lien_image,concat(_utf8'<td>',convert(dossiers_techniques.Pieces.nom using utf8),if((dossiers_techniques.Pieces.quantite > 1),concat(_utf8' (x',cast(dossiers_techniques.Pieces.quantite as char(2) charset utf8),_utf8')'),_utf8''),_utf8'</td>') AS designation,concat(_latin1'<td>',if((dossiers_techniques.Materiaux.URL_wiki = _latin1''),dossiers_techniques.Materiaux.formule,concat(_latin1'<a href="https://fr.wikipedia.org/wiki/',dossiers_techniques.Materiaux.URL_wiki,_latin1'" target="_blank">',dossiers_techniques.Materiaux.formule,_latin1'</a>')),_latin1'</td>') AS matiere,concat(_latin1'<td>',dossiers_techniques.Pieces.observation,_latin1'</td>') AS observation from ((dossiers_techniques.Supports join dossiers_techniques.Pieces) join dossiers_techniques.Materiaux) where ((dossiers_techniques.Pieces.matiere_ID = dossiers_techniques.Materiaux.ID) and (dossiers_techniques.Supports.ID = dossiers_techniques.Pieces.support_ID)) order by dossiers_techniques.Supports.ID,dossiers_techniques.Pieces.repere;
