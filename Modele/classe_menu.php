...
public function __construct($support, $item, $sous_item, $BD) {	// constructeur
...
$T_items = $this->BD->Liste_item($this->ID_support, $this->item);
if(isset($T_items)) {
	$code = '<ul>'."\n";
	foreach($T_items as $i => $item) {	// affichage du menu
		...
		$T_sous_items = $this->BD->Liste_sous_item($this->ID_support, $this->item, $this->sous_item);
